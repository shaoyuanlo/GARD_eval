...
class AdaptiveBatchNorm(nn.Module):
    ...
    def forward(self, input, prior):
        ...
        y = torch.flatten(prior, 1)
        style = self.style(y).unsqueeze(2).unsqueeze(3).unsqueeze(4)
        gamma, beta = style.chunk(2, 1)

        out = self.norm(input)
        out = torch.std(y) * out + torch.mean(y)
        #out = std * self.norm(input) + mu
        out = gamma * out + beta  # channel-wise
        #print(out.shape)
        #print(y.shape)
        #print(torch.std(y).shape)
        #print(gamma.shape)
        #jjj=kkk
...
class ResNeXt(nn.Module):
    ...
    def __init__(self,
        ...
        # --- DPG ---
        self.adain_0 = AdaptiveBatchNorm(64, 1003520)  # 64 x 20x28x28 = 1003520
        self.adain_1 = AdaptiveBatchNorm(256, 1003520)  # 64 x 20x28x28 = 1003520
        self.adain_2 = AdaptiveBatchNorm(512, 250880)  # 128 x 10x14x14 = 250880
        self.adain_3 = AdaptiveBatchNorm(1024, 62720)  # 256 x 5x7x7 = 62720
        self.adain_4 = AdaptiveBatchNorm(2048, 24576)  # 512 x 3x4x4 = 24576
    ...
    def forward(self, x, prior):
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        x = self.adain_0(x, prior[0])

        x = self.layer1(x)
        x = self.adain_1(x, prior[1])
        x = self.layer2(x)	
        x = self.adain_2(x, prior[2])
        x = self.layer3(x)
        x = self.adain_3(x, prior[3])
        x = self.layer4(x)
        x = self.adain_4(x, prior[4])
