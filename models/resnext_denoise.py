...
class Denoising(nn.Module):
    ...
    def __init__(self, n_in):
        ...
        self.conv_theta = nn.Conv3d(n_in, int(n_in/2), 1, bias=False)
        self.conv_phi = nn.Conv3d(n_in, int(n_in/2), 1, bias=False)
        self.conv_out = nn.Conv3d(n_in, n_in, 1, bias=False)
        self.bn_out = nn.BatchNorm3d(n_in)
        self.relu = nn.ReLU(inplace=True)		


    def forward(self, x):
        n_frames, H, W = x.shape[2:]

        #'''
        theta = self.conv_theta(x)
        phi = self.conv_phi(x)		
	    
        f = torch.einsum('nijab,nijcd->njabcd', theta, phi)
        orig_shape = f.shape				
        f = torch.reshape(f, (-1, n_frames, H*W, H*W))	
		
        f = f / torch.sqrt(torch.tensor([theta.shape[1]], dtype=torch.float32).cuda())
        f = F.softmax(f, dim=3)  # dimension!!!	
        f = torch.reshape(f, orig_shape)		
        f = torch.einsum('njabcd,nijcd->nijab', f, x)				
        #'''
        #f = torch.einsum('nikhw,njkhw->nkij', x, x)
        #f = torch.einsum('nkij,nikhw->njkhw', f, x)
        #f = f / torch.tensor([H * W], dtype=torch.float32).cuda()		
...
class ResNeXt(nn.Module):
    ...
    def __init__(self, block, layers, sample_size, sample_duration, shortcut_type='B', cardinality=32, num_classes=400):
        ...
        self.denoise_res2 = Denoising(256)
        self.denoise_res3 = Denoising(512)
        self.denoise_res4 = Denoising(1024)
        self.denoise_res5 = Denoising(2048)		
