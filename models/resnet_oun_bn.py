...
class OUCD_3(nn.Module):
    ...
    def __init__(self, inCh):
        ...
        # BN
        self.conv_1x1a_bn = nn.BatchNorm3d(outCh)
        self.enc_k1_bn = nn.BatchNorm3d(outCh*2)
        self.enc_k2_bn = nn.BatchNorm3d(outCh*4)
        self.enc_k3_bn = nn.BatchNorm3d(outCh*8)
        self.dec_k1_bn = nn.BatchNorm3d(outCh*4)
        self.dec_k2_bn = nn.BatchNorm3d(outCh*2)
        self.dec_k3_bn = nn.BatchNorm3d(outCh)	
        self.enc_u1_bn = nn.BatchNorm3d(outCh*2)
        self.enc_u2_bn = nn.BatchNorm3d(outCh*4)
        self.enc_u3_bn = nn.BatchNorm3d(outCh*8)
        self.dec_u1_bn = nn.BatchNorm3d(outCh*4)
        self.dec_u2_bn = nn.BatchNorm3d(outCh*2)
        self.dec_u3_bn = nn.BatchNorm3d(outCh)		
