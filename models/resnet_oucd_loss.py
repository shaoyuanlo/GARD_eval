...
class OUCD_3(nn.Module):
    ...
    def forward(self, x):
        ...
        # MSFF
        out_msff_a1 = F.interpolate(out_a1, scale_factor=(1,0.25,0.25), mode='trilinear') 
        out_msff_a1 = F.relu_(out_msff_a1)  # ?,  size=1/2, channel=2		
        out_msff_a2 = F.interpolate(out_a2, scale_factor=(1,0.125,0.125), mode='trilinear')
        out_msff_a2 = self.msff_a2(out_msff_a2)
        out_msff_a2 = F.relu_(out_msff_a2)  # ?,  size=1/2, channel=2 		
        out_msff_a3 = F.interpolate(out_a3, scale_factor=(1,0.0625,0.0625), mode='trilinear')
        out_msff_a3 = self.msff_a3(out_msff_a3)
        out_msff_a3 = F.relu_(out_msff_a3)  # ?,  size=1/2, channel=2

        out_msff_b1 = F.interpolate(out_b1, scale_factor=(1,0.125,0.125), mode='trilinear')
        out_msff_b1 = self.msff_b1(out_msff_b1)
        out_msff_b1 = F.relu_(out_msff_b1)  # ?,  size=1/2, channel=2
        out_msff_b2 = F.interpolate(out_b2, scale_factor=(1,0.25,0.25), mode='trilinear')
        out_msff_b2 = F.relu_(out_msff_b2)  # ?,  size=1/2, channel=2		
        out_msff_b3 = F.interpolate(out_b3, scale_factor=(1,0.5,0.5), mode='trilinear')
