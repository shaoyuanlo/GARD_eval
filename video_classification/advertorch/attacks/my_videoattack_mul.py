...
def perturb_iterative(xvar, yvar, sparse_map, predict, nb_iter, eps, eps_iter, loss_fn,
    ...
    for ii in range(nb_iter):
        ...
        if ord == np.inf:
            ...
            delta.data = clamp(delta.data, 0.8, eps)
...
class PGDAttack(Attack, LabelMixin):
    ...
    def perturb(self, x, sparse_map, y=None):
        """
        ...
        rval, delta = perturb_iterative(
            x, y, sparse_map, self.predict, nb_iter=self.nb_iter,
...
class LinfPGDAttack_mul(PGDAttack):
    """
    ...
    def __init__(
            ...
            targeted=False):
        ord = np.inf
        super(LinfPGDAttack_mul, self).__init__(
