...
def perturb_iterative(xvar, yvar, sparse_map, predict, type_pgd, nb_iter, eps, eps_iter, loss_fn,
    ...
    for ii in range(nb_iter):
        outputs = predict(torch.cat((xvar + delta, xvar + delta, xvar + delta), dim=0), type_pgd)
        loss = loss_fn(outputs, torch.cat((yvar, yvar, yvar), dim=0))
        if minimize:
            loss = -loss
...
class PGDAttack(Attack, LabelMixin):
    ...
    def perturb(self, x, type_pgd, sparse_map, y=None):
        """
        ...
        rval, delta = perturb_iterative(
            x, y, sparse_map, self.predict, type_pgd, nb_iter=self.nb_iter,
...
class LinfPGDAttack_bn(PGDAttack):
    """
    ...
    def __init__(
            ...
            targeted=False):
        ord = np.inf
        super(LinfPGDAttack_bn, self).__init__(
