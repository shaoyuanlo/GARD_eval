...
def perturb_iterative(xvar_tgt, xvar_future, yvar, rect_location, predict, nb_iter, eps, eps_iter, loss_fn,
    ...
    if delta_init is not None:
        ...
        delta = torch.zeros_like(xvar_tgt)

    delta.requires_grad_()
    for ii in range(nb_iter):
        outputs = predict(xvar_tgt + delta, xvar_future)  # or predict(xvar_tgt + delta, xvar_future + delta)		
        '''
		# discard loss_fn
		loss_fn = nn.CrossEntropyLoss(reduction="sum")
		loss = loss_fn(outputs, yvar)
        '''
        _, _, h_pred, w_pred = outputs.size()		
        nb, nc, h_gt, w_gt = yvar.size()		
        outputs = nn.functional.interpolate(outputs, size=(h_gt, w_gt), mode='bilinear')

        # cos_sim loss		
        #loss = (1 + nn.functional.cosine_similarity(outputs, yvar[:,:2])).mean()  # 0 ~ 2

        # epe loss
        '''
        if nc==2:  #if args.big_dataset:   use flow_fwd as the gt
            u_gt, v_gt = yvar[:,0,:,:], yvar[:,1,:,:]		
            u_pred, v_pred = outputs[:,0,:,:], outputs[:,1,:,:]
            epe = torch.sqrt(torch.pow((u_gt - u_pred), 2) + torch.pow((v_gt - v_pred), 2))  # 0 ~ inf
            epe = epe.sum() / (nb * h_gt * w_gt)					
        else:
            u_gt, v_gt, valid = yvar[:,0,:,:], yvar[:,1,:,:], yvar[:,2,:,:]		
            u_pred = outputs[:,0,:,:] * (w_gt/w_pred)
            v_pred = outputs[:,1,:,:] * (h_gt/h_pred)					
            epe = torch.sqrt(torch.pow((u_gt - u_pred), 2) + torch.pow((v_gt - v_pred), 2))  # 0 ~ inf
            epe = epe * valid
            epe = epe.sum() / (valid.sum() + 1e-8)
         '''   
        ...
        loss = ((1 / epe) * 10).mean()  # 0 ~ 1
	
        # Maybe we can compute loss with (1) the consideration of "valid" or (2) optimizing epe error.		
        if minimize:
            loss = -loss
        ...
        if ord == np.inf:
            ...
            delta.data = delta.data * rect_location  # only update the attack on the rect_location			
            delta.data = batch_clamp(eps, delta.data)
            delta.data = clamp(xvar_tgt.data + delta.data, clip_min, clip_max
                               ) - xvar_tgt.data
    ...
    x_adv = clamp(xvar_tgt + delta, clip_min, clip_max)
    return x_adv
...
class PGDAttack(Attack, LabelMixin):
    ...
    def perturb(self, x, x_future, rect_location, y=None):
        """
        ...
        rval = perturb_iterative(
            x, x_future, y, rect_location, self.predict, nb_iter=self.nb_iter,
