...
import sys 
sys.path.append('/home/sylo/SegNet/3D-ResNets-PyTorch/video_classification/spatial_transforms.py')
...
def perturb_iterative(detector, xvar, yvar, sparse_map, predict, nb_iter, eps, eps_iter, loss_fn,
    ...
    for ii in range(nb_iter):
        outputs = detector_and_model(detector, predict, xvar + delta)
        ...
        if ord == np.inf:
            ...
            delta.data = delta.data * sparse_map  # only update the attack on the selected frames			
...
class PGDAttack(Attack, LabelMixin):
    ...
    def perturb(self, detector, x, sparse_map, y=None):
        """
        ...
        rval, delta = perturb_iterative(
            detector, x, y, sparse_map, self.predict, nb_iter=self.nb_iter,
        ...
        return rval.data, delta


class LinfPGDAttack_comb(PGDAttack):
    """
    ...
    def __init__(
            ...
            targeted=False):
        ord = np.inf
        super(LinfPGDAttack_comb, self).__init__(
